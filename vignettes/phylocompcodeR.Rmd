---
title: "Including inter-species measurements in differential expression analysis of RNAseq data with the phylocompcodeR package"
...
`phylocompcodeR` is built on the `r Biocpkg("compcodeR")` R package,
that can generate RNAseq counts data and compare the relative performances of 
various popular differential analysis detection tools (@Soneson2013).

`phylocompcodeR` uses the same framework as the `r Biocpkg("compcodeR")`,
with the added possibility to generate "orthologous gene" (OG) expression 
...
This vignette provides a tutorial on how to use the added functionalities of `phylocompcodeR`.
...
```{r tree, eval = FALSE}
library(ape)
tree <- system.file("extdata", "Stern2018.tree", package = "phylocompcodeR")
tree <- read.tree(tree)
```
