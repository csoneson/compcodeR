...
phylolm.createRmd <- function(data.path, result.path, codefile, 
  ...
  if (lengthNormalization != "gwRPKM") {
    ...
    if (dataTransformation == "log2") {
      ...
      writeLines("data.trans <- DESeq2::varianceStabilizingTransformation(data.norm)", codefile)
    }
  }
  writeLines("rownames(data.trans) <- rownames(count_data)", codefile)
  ## Functions to apply phylolm
  writeLines(c("", "# Wrapper functions"),codefile)
  ff <- deparse(extract_results_phylolm)
  ff[1] <- paste0("extract_results_phylolm <- ", ff[1])
  ...
  writeLines(c(
    ...
    "package.version(cdata) <- paste('limma,', packageVersion('limma'))",
    "analysis.date(cdata) <- date()",
    paste("method.names(cdata) <- list('short.name' = 'phylolm', 'full.name' = '", paste('phylolm', packageVersion('limma'), packageVersion('phylolm'), '.', norm.method, '.', model, '.', ifelse(!is.null(measurement_error), 'me', 'nome'), '.', "lengthNorm.", lengthNormalization, '.', "dataTrans.", dataTransformation, ifelse(!is.null(extraDesignFactors), paste0(".", paste(extraDesignFactors, collapse = ".")), ""), sep = ''), "')", sep = ''),
    "is.valid <- check_compData_results(cdata)",
    "if (!(is.valid == TRUE)) stop('Not a valid compData result object.')",
    paste("saveRDS(cdata, '", result.path, "')", sep = "")),
    codefile)  
